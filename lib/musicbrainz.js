...
var Artist = function (mbid) {
	...
	this.setDataFields(['type', 'name', 'sortName', 'lifeSpan', 'ipi']);
...
mb.search = function(resource, query, filter, options, force, callback){
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}
	...
	if(query && query.length > 0 && filterStr.length > 0){
		uri += encodeURIComponent(query) + encodeURIComponent(' AND ') + filterStr;
    ...
    for(var key in options) {
        optionsStr += '&' + encodeURIComponent(key) + '=' + encodeURIComponent(options[key]);
    }
    uri += optionsStr;
...
mb.searchReleases = function (query, filter, options, force, callback) {
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}

	mb.search('release', query, filter, options, force, function (err, data) {
		if (err) {
			callback(err, data);
			return;
		}
...
mb.searchRecordings = function (query, filter, options, force, callback) {
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}

	mb.search('recording', query, filter, options, function (err, data) {
		if (err) {
			callback(err, data);
			return;
		}
...
mb.searchArtists = function (query, filter, options, force, callback) {
	if (typeof callback === 'undefined') {
		callback = force;
		force = false;
	}

	mb.search('artist', query, filter, options, function (err, data) {
		if (err) {
			callback(err, data);
			return;
		}
